...
def exec_external_pgm(exec_type, exec_path) :
    ...
    print(dt+" Begin: exec_external_pgm")
    ...
    dt = datetime.strftime(datetime.now(), "%Y-%m-%d_%H:%M:%S")
    print(dt+" End: exec_external_pgm", status)
    ...
    if exec_type == "hourly_rates" :
        if fi_hr.is_file():
            ...
            if not status :
                ...
                log_action("exec_external_pgm hourly_rates:\n\t"+g_ui_text["t30h"], True) # No return

            return(True)
        else:
            print(te["fi_hourly_rate"]+" not created")
            log_action("exec_external_pgm:\n\tFailed to create:"+te["fi_hourly_rate"], True) # No return
    ...
    if exec_type == "monthly_rates" :
        if fi_mon.is_file():
            ...
            log_action("exec_external_pgm:\n\tFailed to create:"+te["fi_monthly_rates"]+" monthly_rates not working.", False)
            return(False)
    ...
    if exec_type == "temp_reading" :
        if fi_indoor_temp.is_file():
            ...
            log_action("exec_external_pgm:\n\tFailed "+te["fi_indoor_temp_reading"]+" temperature reading not working.", False)
